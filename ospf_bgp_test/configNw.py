...
def annotate_graph(graph: networkx.Graph) -> networkx.Graph:
    ...
    for name in graph.nodes():
        ...
        if name[1] == "0":
            ...
            node['defaultGateway'] = format(ipaddress.IPv4Address(0x0A010005)) # IP of R0_2

        elif name[1] == "1":
            ip = 0x0A020000 + count # 10.2.0.0
            count += 2
            node["ip"] = ipaddress.IPv4Interface((ip, 31))
            node['defaultGateway'] = format(ipaddress.IPv4Address(0x0A020007)) # IP of R1_0
...
def create_ospf_config(graph: networkx.Graph, name: str) -> str:
    ...
    if ip is not None:
        ...
        networks_str.append(OSPF_NW_TEMPLATE.format(network=ipaddress.IPv4Network(('10.15.0.0', 16))))
        redistribute = ""
        bgp = ""
    ...
    if node["bgp_edge"]:
        ...
        for neighbor in graph.adj[name]:
            ...
            if ip_net in ipaddress.IPv4Network("172.16.0.0/16"):
        ...
        addDefaultGateway = f"ip route 0.0.0.0/0 {format(node['defaultGateway'])}"
