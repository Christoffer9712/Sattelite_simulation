...
control MyIngress(inout headers hdr,
    ...
    // ------------------------------------------------------------------ //
    action no_insrt(bit<9> out_port) {
    ...
    //   NS/RS (host→router)  : no_insrt(2)
    //   NA/RA (router→host)  : no_insrt(1)
    //
    ...
    // ------------------------------------------------------------------ //
    action srv6_insrt_1sid(bit<128> s1, bit<9> out_port) {
        hdr.ipv6.nextHdr = NEXTHDR_SRH;
        ...
        hdr.srv6_list[0].segmentId = hdr.ipv6.dstAddr;
    ...
    action srv6_insrt_2sid(bit<128> s1, bit<128> s2, bit<9> out_port) {
        hdr.ipv6.nextHdr = NEXTHDR_SRH;
    ...
    action srv6_insrt_3sid(bit<128> s1, bit<128> s2, bit<128> s3, bit<9> out_port) {
        hdr.srv6h.setValid();
        hdr.srv6h.routingType = 4;
        hdr.srv6h.segLeft     = 3;
        hdr.srv6h.lastEntry   = 3;
        hdr.srv6h.flags       = 0;
        hdr.srv6h.tag         = 0;
        hdr.srv6h.hdrExtLen   = 8;
        hdr.srv6h.nextHdr = hdr.ipv6.nextHdr;
        ...
        standard_metadata.egress_spec = out_port;
        hdr.ipv6.payloadLen = hdr.ipv6.payloadLen + 72;
    }
    ...
    table srv6_sid_table {
        ...
        const entries = {
            128w0x00fc0020000000000000000000000002 &&& 128w0xffffffffffffffffffffffffffffffff : srv6_insrt_3sid(128w0x00220010000000010000000000000000, 128w0x00220020000000010000000000000000, 128w0x00220030000000010000000000000000, 2);
            128w0x00fc0020000000000000000000000001 &&& 128w0xffffffffffffffffffffffffffffffff : srv6_insrt_2sid(128w0x00220010000000020000000000000000, 128w0x00220030000000010000000000000000, 2);
            128w0x00fa0000000000000000000000000001 &&& 128w0xffffffffffffffffffffffffffffffff : no_insrt(1);
        }
    }
